...
use crate::engine_rest::types::common::{Bytes20, Withdrawal};
...
fn ssz_withdrawal(w: &Withdrawal) -> InternalWithdrawal {
    InternalWithdrawal {
        index: w.index,
...
// ── Helper: fill JSON payload from the Osaka/Amsterdam payload shape ──────────
...
fn payload_fields_to_json(f: CommonPayloadFields) -> Result<JsonExecutionPayload, ProblemJson> {
...
fn osaka_payload_to_json(p: osaka::ExecutionPayload) -> Result<JsonExecutionPayload, ProblemJson> {
    let withdrawals: Vec<InternalWithdrawal> = p.withdrawals.iter().map(ssz_withdrawal).collect();
    payload_fields_to_json(CommonPayloadFields {
...
impl IntoEngineCall for osaka::ExecutionPayloadEnvelope {
    fn into_engine_call(self) -> Result<DecodedNewPayload, ProblemJson> {
        ...
        let json = osaka_payload_to_json(self.execution_payload)?;
        let block = json
...
fn amsterdam_payload_to_json(
    ...
    let json = payload_fields_to_json(CommonPayloadFields {
        parent_hash: p.parent_hash,
        fee_recipient: p.fee_recipient,
        state_root: p.state_root,
        receipts_root: p.receipts_root,
